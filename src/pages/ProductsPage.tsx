...
const products = [{
  ...
  icon: Compass,
  color: "#f59e0b",
  image: droneSurveillance, // Reuse existing image
...
const DroneVisual = memo(({ product, isHovered }: { product: typeof products[0]; isHovered: boolean }) => {
  return (
    <div className="relative w-full aspect-square max-w-md mx-auto">
      ...
      <div 
        ...
        style={{ backgroundColor: product.color, opacity: 0.6 }}
      />
      <div 
        className="absolute bottom-1/3 right-1/4 w-1.5 h-1.5 rounded-full animate-pulse"
        style={{ backgroundColor: product.color, opacity: 0.4, animationDelay: '0.5s' }}
      />
    </div>
  );
});
...
const ProductSection = memo(({ product, index }: { product: typeof products[0]; index: number }) => {
    ...
    >
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
          {/* Visual - Static image with CSS effects */}
          <div className={`relative ${isEven ? "" : "lg:order-2"}`}>
            <DroneVisual product={product} isHovered={isHovered} />
          </div>
          
          {/* Content - Prioritized for FCP */}
          <div className={isEven ? "" : "lg:order-1"}>
            ...
            <div className="flex items-center gap-4 mb-5">
              ...
              <span 
                className="text-sm font-display uppercase tracking-[0.15em] font-medium" 
